...
public class IndustryEvaluationUploadService {
    ...
    @Transactional
    public void importIndustryEvaluations(String filePath, Long roundId) throws IOException {
        ...
        try (BufferedReader reader = Files.newBufferedReader(Paths.get(filePath), Charset.forName("UTF-8"));
            ...
            for (String header : csvParser.getHeaderNames()) {
                if (header.contains("original_product_id")) actualDataCodeKey = header;
            ...
            for (CSVRecord record : csvParser) {
                ...
                // DB 조회
                IndustryData industryData = industryDataRepository.findByOriginalId(data_code)
                ...
                for (String header : csvParser.getHeaderNames()) {
                    if (header.matches("^\\d+\\..*")) {
                        ...
                        IndustrySurvey is = industrySurveyRepository.findBySurveyCode(surveyCode)
                        ...
                        industryResponseRepository.save(IndustryResponse.builder()
                ...
                // 5. 정성평가(VI_TEXT) 저장
                IndustrySurvey textSurvey = industrySurveyRepository.findBySurveyCode("PR_TEXT")
                        .orElseThrow(() -> new RuntimeException("PR_TEXT 문항 없음"));

                industryResponseRepository.save(IndustryResponse.builder()
                        .industryData(industryData)
                        .industrySurvey(textSurvey)
