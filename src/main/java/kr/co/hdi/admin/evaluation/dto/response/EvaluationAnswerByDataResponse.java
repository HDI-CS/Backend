...
public record EvaluationAnswerByDataResponse(
        Long dataId,
        String dataCode,
        List<EvaluationAnswerResponse> surveys
) {
    public static EvaluationAnswerByDataResponse ofIndustry(
            DataIdCodePair pair,
    ...
    public static EvaluationAnswerByDataResponse ofVisual(
            DataIdCodePair pair,
            List<VisualSurvey> surveys,
            List<VisualResponse> responsesForData
    ) {
        Map<Long, VisualResponse> responseBySurveyId = responsesForData.stream()
                .collect(Collectors.toMap(
                        r -> r.getVisualSurvey().getId(),
                        r -> r
                ));

        List<EvaluationAnswerResponse> answers = surveys.stream()
                .sorted(Comparator.comparing(VisualSurvey::getSurveyNumber))
                .map(s -> EvaluationAnswerResponse.fromSurveyAndResponse(
                        s,
                        responseBySurveyId.get(s.getId())
                ))
